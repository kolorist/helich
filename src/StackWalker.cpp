...
BOOL StackWalker::ShowCallstack(cstr outBuffer, HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
	CONTEXT c;;
...
#error "Platform not supported!"
	...
	for (frameNum = 0; ; ++frameNum)
	{
		...
		if (outBuffer) {
			c8 traceLine[1024]; traceLine[0] = 0;
			this->OnCallstackEntry(et, csEntry, traceLine);
			if (traceLine[0] != 0)
				strcat(outBuffer, traceLine);
		}
		else {
		...
		if (s.AddrReturn.Offset == 0)
		{
			if (outBuffer) {
				c8 traceLine[1024]; traceLine[0] = 0;
				this->OnCallstackEntry(lastEntry, csEntry, traceLine);
				if (traceLine[0] != 0)
					strcat(outBuffer, traceLine);
			}
			else {
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, cstr outBuffer /* = NULL */)
{
	CHAR buffer[STACKWALK_MAX_NAMELEN];
